...
\item Per calcolare $P^{-1}\mathbf{r}^{(k)}$, non serve calcolare l'inversa, bensì basta risolvere il sistema $\mathbf{z}^{(k)} =P^{-1}\mathbf{r}^{(k)}$\footnote{In generale, l'inversione di matrice è un'operazione dispendiosa e potenzialmente instabile dal punto di vista numerico, quindi si cerca di utlizzarla il meno possibile, preferendo risoluzioni di sistemi lineari come nell'esempio appena mostrato.}.
...
\begin{algo}
	...
	calcola $\mathbf{r}^{(0)}$\;
	\For{$k=0,1,2,\dotsc $}{
		calcola del residuo precondizionato $P\mathbf{z}^{(k)} =\mathbf{r}^{(k)}$\;
...
\end{equation*}
Infatti, il metodo del gradiente non tiene traccia di tutte le direzioni precedenti, ma solo di quella immadiatamente precedente.
